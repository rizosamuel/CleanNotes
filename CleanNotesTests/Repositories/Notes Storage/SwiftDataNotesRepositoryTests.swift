//
//  SwiftDataNotesRepositoryTests.swift
//  CleanNotes
//
...
@available(iOS 17, *)
class SwiftDataNotesRepositoryTests: XCTestCase {
    
    var sut: SwiftDataNotesRepositoryImpl!
    var container: ModelContainer!
    
    @MainActor override func setUp() {
        super.setUp()
        container = try! ModelContainer(for: SwiftDataNote.self)
        sut = SwiftDataNotesRepositoryImpl(container: container)
        sut.removeAllNotes()
    }
    ...
    @MainActor func test_fetchNotes_initiallyEmpty() {
        let expectation = self.expectation(description: "Fetch notes after adding a note")
        sut.fetchNotes { fetchedNotes in
    ...
    @MainActor func test_fetchNotes_afterAddingNotes() {
